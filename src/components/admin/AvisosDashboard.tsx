...
export default function AvisosDashboard({ ads }: AvisosDashboardProps) {
  ...
  const clients = useMemo(() => {
    ...
    for (const ad of ads) {
      if (ad.client_id && ad.client_name && !map.has(ad.client_id)) {
        map.set(ad.client_id, ad.client_name);
      }
    }
  ...
  const filtered = useMemo(() => {
    ...
    return ads.filter((ad) => {
      ...
      if (selectedClient !== "all" && ad.client_id !== selectedClient) return false;
      if (expiringSoon) {
      ...
      if (search !== "") {
        const matchesSearch =
          ...
          (ad.client_name || "").toLowerCase().includes(searchLower);
        if (!matchesSearch) return false;
      }
      return true;
    });
  }, [ads, selectedType, selectedClient, expiringSoon, search]);
  ...
  return (
    <>
        ...
        toolbarRight={
          <>
            <button
              ...
              className={`px-3 py-2 text-xs font-bold tracking-wide rounded whitespace-nowrap transition-colors ${
                selectedType === "all"
                  ? "bg-ink text-white"
      ...
      <AdTypeSelector ads={ads} selectedType={selectedType} onSelectType={setSelectedType} />
      ...
      <div className="flex items-center gap-3 flex-wrap mb-4 -mt-3">
        ...
        <button
          ...
          title="Banners que vencen en los próximos 7 días"
        >
        ...
        <span className="text-xs text-muted ml-auto">
          {filtered.length} de {ads.length} avisos
        </span>
      </div>

      {filtered.length === 0 ? (
        <div className="bg-paper rounded-lg p-12 text-center border border-border">
          <p className="text-muted">No se encontraron avisos con los filtros actuales.</p>
        </div>
      ) : viewMode === "cards" ? (
